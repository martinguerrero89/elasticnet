...
print(paste("baseline correlation is", mean(geneC),sep=" "))
...
print(paste("baseline correlation without NAs is", mean(geneCc),sep=" "))
...
for (i in 1:nrow(trainY)){
  ...
  cvfit = cv.glmnet(Xtrain,as.numeric(trainY[i,]), parallel = TRUE,type.measure = "mae",alpha=0.5,nfold=ncol(trainY))
...
print(paste("predicted correlation is", mean(geneCc2),sep=" "))


COEF2= list()
for (i in 1:nrow(trainY)){
  
  cvfit = cv.glmnet(predt,as.numeric(trainY[i,]), parallel = TRUE,type.measure = "mae",nfold=nrow(Xtrain))
  Cof= coef(cvfit, s = "lambda.min")
  colnames(Cof)=rownames(trainY)[i]
